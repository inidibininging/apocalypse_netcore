...
namespace Apocalypse.Any.Client.States
{
    public class BuildGameSheetFramesState : IState<string, INetworkGameScreen>
    {
        ...
        public void Handle(IStateMachine<string, INetworkGameScreen> machine)
        {
            ...
            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("projectile", 32, 32, 1, 5, 10, 10))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);
            ...
            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("dialogue", 32, 32, 0, 0, 0, 4))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);
            ...
            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("hud_misc_edit", 32, 32, 0, 8, 0, 8))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);

            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("miniCity", 32, 32, 1, 6, 5, 5))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);

            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("miniCity", 32, 32, 0, 5, 0, 0))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);

            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("miniCity", 32, 32, 0, 6, 1, 1))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);

            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("miniCity", 32, 32, 0, 2, 2, 2))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);

            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("miniCity", 32, 32, 3, 6, 2, 2))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);

            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("miniCity", 32, 32, 0, 6, 3, 4))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);

            foreach (var currentFrame in FrameGeneratorService.GenerateGameSheetAtlas("miniCity", 32, 32, 0, 5, 4, 0))
                machine.SharedContext.GameSheet.Frames.Add(currentFrame.Key, currentFrame.Value);
