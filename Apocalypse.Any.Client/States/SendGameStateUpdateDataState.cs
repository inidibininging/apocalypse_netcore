...
namespace Apocalypse.Any.Client.States
{
    ...
    public class SendGameStateUpdateDataState : IState<string, INetworkGameScreen>
    {
        ...
        public void Handle(IStateMachine<string, INetworkGameScreen> machine)
        {
            ...
            cmds.ForEach(cmd => { machine.SharedContext.Messages.Add(cmd); });

            if (machine.SharedContext.LastMetadataBag != null && machine.SharedContext.LastMetadataBag.Stats != null &&
                machine.SharedContext.LastMetadataBag.Stats.Health <=
            ...
            else
            {
                ...
                {
                    var selectedDialog =
                        machine.SharedContext.LastMetadataBag.CurrentDialog?.DialogIdContent.FirstOrDefault(d =>
                            d.Item2 == machine.SharedContext.LastMetadataBag.ClientEventName);
                    if (selectedDialog != null) cmds.Add($"{DefaultKeys.OpenDialog} {selectedDialog.Item1}");
                    machine.SharedContext.LastMetadataBag.ClientEventName = null;
                }
            }
        ...
        private NetOutgoingMessage CreateMessage<T>(IStateMachine<string, INetworkGameScreen> machine, byte commandName,
            T instanceToSend)
        {
