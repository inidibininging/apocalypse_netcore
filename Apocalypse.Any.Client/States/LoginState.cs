...
namespace Apocalypse.Any.Client.States
{
    ...
    public class LoginState : IState<string, INetworkGameScreen>
    {
        ...
        {
            if (string.IsNullOrEmpty(ip)) throw new ArgumentException("message", nameof(ip));
        ...
        public void Handle(IStateMachine<string, INetworkGameScreen> machine)
        {
            ...
            while (machine.SharedContext.LoginSendResult != NetSendResult.Sent)
            {
                ...
                var user = CreateMessage(NetworkCommandConstants.LoginCommand,
                    machine.SharedContext.Configuration.User);
                var outgoingMessage = machine.SharedContext.Client.CreateMessage();
                outgoingMessage.Write(user);
                machine.SharedContext.LoginSendResult =
                    machine.SharedContext.Client.SendMessage(outgoingMessage, NetDeliveryMethod.Unreliable);
