...
namespace Apocalypse.Any.Client.States.UI.Dialog
{
    public class UpdateDialogHoverTextState : IState<string, INetworkGameScreen>
    {
        public void Handle(IStateMachine<string, INetworkGameScreen> machine)
        {
            ...
            var hoveredText = machine.SharedContext.DialogWindow.Values.OfType<VisualText>()
                .FirstOrDefault(inventoryItemImage =>
                    Vector2.Distance(cursorImageAsVector, inventoryItemImage.Position) < 32);
            ...
            var dialog =
                machine.SharedContext.LastMetadataBag?.CurrentDialog.DialogIdContent?.FirstOrDefault(dlg =>
                    dlg.Item2 == hoveredText.Text);
            if (dialog == null)
            {
            }
