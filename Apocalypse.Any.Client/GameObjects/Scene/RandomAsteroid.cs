...
namespace Apocalypse.Any.Client.GameObjects.Scene
{
        ...
        IUpdateableLite
    {
        ...
        public RandomAsteroid()
        {
            ...
            LayerDepth = Randomness.Instance.TrueOrFalse()
                ? DrawingPlainOrder.Entities - DrawingPlainOrder.PlainStep
                : DrawingPlainOrder.Entities + DrawingPlainOrder.MicroPlainStep;
            RotateRight = Randomness.Instance.TrueOrFalse();
        }

        public Vector2 Scale { get; set; } = new(Randomness.Instance.From(10, 200) / 150);
        ...
        public void Update(GameTime gameTime)
        {
            ...
            Position.X +=
                Destination.X > Position.X
                    ? SingleUnit
                    : -SingleUnit; //Speed.X;//Destination.X > Position.X ? SingleUnit : -SingleUnit;//Speed.X;//Destination.X > Position.X ? SingleUnit : -SingleUnit;
            Position.Y +=
                Destination.Y > Position.Y
                    ? SingleUnit
                    : -SingleUnit; //Speed.Y;//Destination.Y > Position.Y ? SingleUnit : -SingleUnit;//Speed.Y;//Destination.Y > Position.Y ? SingleUnit : -SingleUnit;            
